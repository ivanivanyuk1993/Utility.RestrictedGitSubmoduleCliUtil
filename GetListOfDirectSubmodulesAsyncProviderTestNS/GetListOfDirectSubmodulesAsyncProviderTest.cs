...
public class GetListOfDirectSubmodulesAsyncProviderTest
{
    ...
    public async Task TestMethod1()
    {
        ...
        var listOfDirectSubmodulesOrError = await GetListOfDirectSubmodulesAsyncProvider.GetListOfDirectSubmodulesAsync(
            console: systemConsole,
            gitRootDirectoryInfo: new DirectoryInfo(path: AppDomain.CurrentDomain.BaseDirectory),
            cancellationToken: cancellationToken
        );

        listOfDirectSubmodulesOrError.RunActionWithValueOrError(
            runActionWithValueFunc: listOfDirectSubmodules =>
            {
                Assert.AreEqual(
